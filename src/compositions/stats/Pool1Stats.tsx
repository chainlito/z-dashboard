...
const Pool1Stats = ({ tokenPrice, account }: Props) => {
  ...
  useEffect(() => {
    ...
    dexclient.getYTokenPrice().then(res => setToken1Price(res));
  });
  useEffect(() => {
    if (token1Price > 0) {
  ...
  return (
    <React.Fragment>
      <h2 className='mb-20 ystatstitle'>{Config.YToken.name} staking pool</h2>
	    <div className='cardoutline'>
        <div className='ystatshead'>PRICES <span className='ybullets'> • • • • • • • • • • • • • • • • • • • • • • • • • •</span></div>
	      <div className='flex-h'>
          <div className='datasplit'>{`1 ${Config.YToken.symbol} =`}</div>
        ...
        <div className='ystatshead'>STAKING <span className='ybullets'> • • • • • • • • • • • • • • • • • • • • • • • • • •</span></div>	 
          ...
          <div className='blackdata'>
            {`${numberWithDecimals(totalStaked, Config.YToken.decimals, Config.Utils.decimals)} ${Config.YToken.symbol} staked in ${Config.Token.symbol}'s ${Config.YToken.symbol} staking pool.`}
            <br/>
            {`= $ ${numberWithDecimals(totalStaked * token1Price, Config.YToken.decimals, 2)}`}
          </div>
	      </div>
	      <div className='flex-h'>
          <div className='datasplit'>
            You are staking &nbsp;
		      ...
		      <div className='blackdata'>
            {`${numberWithDecimals(staked, Config.YToken.decimals, Config.Utils.decimals)} ${Config.YToken.symbol} (${(staked / totalStaked * 100).toFixed(2)}% of the pool)`}
		        <br/>
		        {`= $ ${numberWithDecimals(staked * token1Price, Config.YToken.decimals, 2)}`}
          </div>
	      </div>
        <br/>
