...
const Pool2Stats = ({ tokenPrice, account }: Props) => {
  ...
  useEffect(() => {
		web3client.poolGetRewardRate(web3client.pool2Contract).then(res => {
			if (tokenPrice > 0) {
				const roi = res / totalStaked * tokenPrice / token1Price;
				setRoiUnit(roi);
			}
			setRate(res * staked / totalStaked / Math.pow(10, 18));
		});
	}, [tokenPrice, staked, token1Price, totalStaked]);
	
  return (
    <React.Fragment>
      <h2 className='mb-20 ystatstitle'>{Config.UniLpToken.name} staking pool</h2>
      		...
      		<div className='datasplit'>{`1 ${Config.UniLpToken.symbol} =`}</div>
				...
				<div className='flex-h'>
					...
					<div className='blackdata'>
						{`${numberWithDecimals(totalStaked, Config.UniLpToken.decimals, Config.Utils.decimals)} ${Config.UniLpToken.symbol} staked in ${Config.Token.symbol}'s ${Config.UniLpToken.symbol} staking pool.`}
						<br/>
						{`= $ ${numberWithDecimals(totalStaked * token1Price, Config.UniLpToken.decimals, 2)}`}
					</div>
				</div>
				<div className='flex-h'>
					<div className='datasplit'>
					...
					<div className='blackdata'>
						{`${numberWithDecimals(staked, Config.UniLpToken.decimals, Config.Utils.decimals)} ${Config.UniLpToken.symbol} (${(staked / totalStaked * 100).toFixed(2)}% of the pool)`}
						<br/>
						{`= $ ${numberWithDecimals(staked * token1Price, Config.UniLpToken.decimals, 2)}`}
					</div>
				</div>
				<br/>
